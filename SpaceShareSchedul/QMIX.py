...
class DRQN(nn.Module):
    def __init__(self, conf, device = None):
        ...
        self.conf = conf
        fact = 30
        rnn_dim = self.conf.drqn_hidden_dim
        self.rnn_dim = rnn_dim
        self.input_layer = nn.Linear(self.conf.obs_shape, rnn_dim)
        self.rnn = nn.GRUCell(rnn_dim, rnn_dim)
        self.output_layer = nn.Linear(rnn_dim, self.conf.output_shape)
    ...
    # def forward_item(self, x, h):
    #     x = F.relu(self.fc1(x))
    #     h_in = h.reshape(-1, self.rnn_dim)
    #     h = self.rnn(x, h_in)
    #     q = self.output_layer(h)
...
class QMIX(object):
    ...
    def init_hidden(self, episode_num):
        self.eval_hidden = torch.zeros((episode_num, self.conf.n_agents, self.conf.drqn_hidden_dim))  # 第一个参数暂时按线程数看待， 暂时赋值1
        self.target_hidden = torch.zeros((episode_num, self.conf.n_agents, self.conf.drqn_hidden_dim))
...
class QMIXMemory(object):
    ...
    def sample(self, num):
        ...
        o, h, u, s, a, r, o_, u_, s_, done = [], [], [], [], [], [], [], [], [], []
        result = (o, h, u, s, a, r, o_, u_, s_, done)
    ...
    def handle_track(self, tracks):
        ...
        self.push((o, h, u, s, a, r, o_, u_, s_, done))
        i = l - 2
        #
        while i >= 0:
            o = tracks[i]['s'][0]
...
class Schedule(object):
    ...
    def getQValues(self, sample):
        ...
        o, h, u, s, a, r, o_, u_, s_, done = sample
        ...
        q, qh =self.qmix.eval_drqn_net(o1, h1)
        q2 = q.view(*o_shape[:-1])
        qt, qht = self.qmix.target_drqn_net(o1, h1)
    ...
    def train(self, memory):
        ...
        for i in range(recycle):
            ...
            o, h, u, s, a, r, o_, u_, s_, done = sample_result
    ...
    def getS(self, cluster, index=0):
        ...
        return [X, self.qmix.eval_hidden[0], available_mc, S], choose_mc_list
    ...
    def selectAction(self, X):
        ...
        u = X[2]
        available_mc = X[2]
        a, h = self.qmix.eval_drqn_net(X0, X1)
        available_mc = np.array(available_mc)
        a[available_mc == 0] = -float('inf')
